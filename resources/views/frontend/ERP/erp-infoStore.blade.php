            ...
            <div id="content-erp">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <div class="form-group row">
                            {{--<div class="col-sm-12">--}}
                                {{--<button class="btn btn-sm btn-secondary float-right" data-toggle="modal" data-target="#createProductModal"><i class='fas fa-plus'></i> เพิ่มสินค้า</button>--}}
                            {{--</div>--}}
                        </div>
                        @if($store_air != null)
                            <div class="card-deck">
                                @foreach($store_air as $key => $value)
                                <div class="col-sm-4">
                                    <div class="card border-warning rounded-0 test-articlebox" style="margin-bottom: 5px;">
                                        <a>
                                            <div class="card-body">
                                                <div class=" img-cover-testarticle">
                                                    <img src="{{ asset('images/store-product/'.$value->pa_image) }}" class="mw-100 img-article2">
                                                </div>
                                                <h5 class="card-title"><i class="fas fa fa-info-circle"></i> {{ $value->pa_name }}</h5>
                                                <small>
                                                    <span>
                                                ...
                                                <div class="">
                                                    <a href="{{ url('erpEditStoreProduct/'.$value->id) }}" class="btn btn-outline-info btn-sm">แก้ไข</a>
                                                    <a href="{{ url('deleteProduct/'.$value->id) }}" class="btn btn-outline-danger btn-sm">ลบข้อมูล</a>
                                                    <a href="{{ url('erpInfoStoreProduct/'.$value->id) }}"
                                                       class="btn btn-outline-secondary btn-sm">ดูข้อมูล</a>
                                                    <hr>
                                                    <i class="fas fa fa-money" title="ราคา"></i> {{ $value->air_price_list }}
                                                    <i class="fas fa fa-shopping-cart" title="สินค้าคงเหลือ ใน stock"></i> {{ $value->pa_stock }}
                                                </div>
                                            </div>
                                        </a>
                                    </div>
