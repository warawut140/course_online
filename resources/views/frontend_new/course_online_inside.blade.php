...
<body>
    ...
    <video style="width: 100%;" id="myvideo" controls controlsList="nodownload"
        src="{{ asset('images/profile/' . $list->course_video) }}" autoplay="autoplay">
    </video>
    ...
    <div class="opcenter">
        <div class="card-body">
            ...
            <?php
            $chapter = \DB::table('courses_chapter')
                ->where('id', $list->chapter_id)
                ->first();
            ?>
            <div class="container">
            ...
            <div class="d-flex align-items-center" id="curriculumheadingOne">
                <h5 class="mb-0 w-100">
                    <button
                        ...
                        บทเรียนที่ {{ $chapter->order }} {{ $chapter->name }}
                    </button>
                </h5>
            </div>
            @foreach ($course_list as $key => $l)
                <div id="CurriculumcollapseOne{{ $key }}" class="collapse show"
                    aria-labelledby="curriculumheadingOne{{ $key }}" data-parent="#accordionCurriculum">
                    <div class="border-top px-5 py-4 min-height-70 d-md-flex align-items-center">
                        ...
                        question
                            ...
                            <a href="{{ url('course_online_inside_view/' . $l->id) }}" class="text-secondary d-flex">
                                <!-- Icon -->
