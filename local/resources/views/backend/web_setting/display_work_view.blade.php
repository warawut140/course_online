...
@section('content')
    ...
    <div class="content">
        ...
        <!-- BEGIN: Wizard Layout -->
        <form method="POST" action="{{ url('admin/display_work/store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="intro-y box py-10 sm:py-15 mt-5">
                <div class="px-5 mt-0">
                    <div class="font-medium text-left text-lg">จัดการงาน</div>
                ...
                <div class="px-5 sm:px-20 mt-10 pt-10 border-t border-slate-200/60 dark:border-darkmode-400">
                    <div class="font-medium text-base">ข้อมูลงาน</div>
                    <div class="grid grid-cols-12 gap-4 gap-y-5 mt-5">
                        <div class="intro-y col-span-12 sm:col-span-6">
                            <label for="input-wizard-1" class="form-label">บริษัท</label>
                            <input id="input-wizard-1" name="name" type="text" class="form-control"
                                value="{{ @$data->Profile->company }}" readonly placeholder="กรุณาระบุข้อมูล">
                        </div>
                        <div class="intro-y col-span-12 sm:col-span-6">
                            <label for="input-wizard-1" class="form-label">ตำแหน่งงาน</label>
                            <input id="input-wizard-1" name="name" type="text" class="form-control"
                                value="{{ @$data->position }}" readonly placeholder="กรุณาระบุข้อมูล">
                        </div>
                        ...
                        @if (@$data)
                            <div class="intro-y col-span-12 sm:col-span-12">
                                <img src="{{ asset('images/profile/' . @$data->Profile->image_profile) }}" class="circleco mb-2 mw-100"
                                    width="400" height="400">
                            </div>
                        @endif

                        <div class="intro-y col-span-12 sm:col-span-12">
                            <label for="input-wizard-1" class="form-label">แสดงหลักสูตร &nbsp;</label>
                           <select class="form-caontrol" id="input-wizard-1" name="status">
                            <option value="1" <?php if(@$data->status==1){echo 'selected';} ?>>เปิด</option>
                            <option value="0" <?php if(@$data->status==0){echo 'selected';} ?>>ปิด</option>
                           </select>
                        </div>

                        <div class="intro-y col-span-12 flex items-center justify-center sm:justify-end mt-5">
                            <a href="{{ url('admin/display_course') }}" class="btn btn-secondary w-24">ย้อนกลับ</a>
