    ...
    <div class="content">
        <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
            <h2 class="text-lg font-medium mr-auto">
                จัดการข้อมูลแบนเนอร์
            </h2>
            <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
                <a class="btn btn-primary shadow-md mr-2" href="{{url('admin/banner/add')}}"><span class="w-5 h-5 flex items-center justify-center"> <i
                            class="w-4 h-4" data-lucide="plus"></i> </span>เพิ่มข้อมูล</a>
                ...
                <div class="preview">
                    <div class="overflow-x-auto">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                @foreach ($banners as $key => $st)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ @$st->name }}</td>
                                        <td> <img src="{{ asset('images/banner/'.@$st->image) }}" class="circleco mb-2 mw-100"
                                                width="100" height="100"></td>
                                        <td>{{ @$st->created_at }}</td>
                                        <td><a href="{{ url('admin/banner/view/' . $st->id) }}"
